...
int main(int argc, char *argv[])
{
  ...
  int **rows = malloc(height_dec * sizeof(int *));
  if (rows == NULL)
  {
  ...
  {
    int *pixel_data = malloc(width_dec * sizeof(int));
    if (pixel_data == NULL)
    {
  ...
  int **rows_blur = malloc(height_dec * sizeof(int *));
  if (rows_blur == NULL)
  {
  ...
  for (int h = 0; h < height_dec; h++)
  {
    int *row_blur = malloc(width_dec * sizeof(int));
    if (row_blur == NULL)
    {
    ...
    for (int w = 0; w < width_dec; w++)
    {
      ...
      for (int dh = -4; dh <= 4; dh++)
      {
        for (int dw = -4; dw <= 4; dw++)
        {
          ...
          {
            brightness_sum += rows[new_h][new_w];
            count++;
          }
        }
      }
      
      rows_blur[h][w] = round((float)brightness_sum / count);
    }
  }
  ...
  int **small_rows = malloc(new_height * (sizeof(int *)));
  if (small_rows == NULL)
  {
  ...
  {
    int *small_row = malloc(new_width * (sizeof(int)));
    if (small_row == NULL)
    {
  ...
  for (int h = new_height - 1; h > -1; h--)
  {
    for (int w = 0; w < new_width; w++)
    {
      if (small_rows[h][w] > 200)
      {
        fprintf(output, "@@");
      }
      else if (small_rows[h][w] > 150 && small_rows[h][w] < 200)
      {
        fprintf(output, "##");
      }
      else if (small_rows[h][w] > 100 && small_rows[h][w] < 150)
      {
        fprintf(output, "&&");
      }
      else if (small_rows[h][w] > 50 && small_rows[h][w] < 100)
      {
        fprintf(output, "..");
      }
      else
